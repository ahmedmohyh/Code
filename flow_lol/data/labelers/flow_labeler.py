...
class FlowLabeler:
    ...
    def fit(self, scores: np.ndarray) -> "FlowLabeler":
        """Compute thresholds on the available scores."""
        ...
        self.percentile_low_threshold_ = float(np.nanquantile(scores, self.config.percentile_low))
        self.percentile_high_threshold_ = float(np.nanquantile(scores, self.config.percentile_high))
        return self
    ...
    def transform(self, scores: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        that are not inside the exclusion margin or outside the selected
        percentile range.
        """
        scores = np.asarray(scores, dtype=float)
        ...
        if self.config.method == "extreme_percentile":
            low = scores <= self.percentile_low_threshold_
            high = scores >= self.percentile_high_threshold_
            excluded = (scores > self.percentile_low_threshold_) & (scores < self.percentile_high_threshold_)
        else:
    ...
    def describe(self) -> dict:
        return {
            ...
            "percentile_low_threshold": getattr(self, "percentile_low_threshold_", None),
            "percentile_high_threshold": getattr(self, "percentile_high_threshold_", None),
        }
